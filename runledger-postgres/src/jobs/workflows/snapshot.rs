use runledger_core::jobs::{JobStage, StepKey, WorkflowRunEnqueue, WorkflowStepEnqueue};
...
/// The single normalized representation of a current append request.
///
/// It is serialized for persistence and compared directly with legacy rows
/// decoded through the tolerant schema on this same type.
#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
pub(super) struct CanonicalAppendRequest {
    #[serde(default)]
    pub(super) append_window_step_key: Option<String>,
    pub(super) steps: Vec<CanonicalWorkflowStep>,
}

#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
pub(super) struct CanonicalWorkflowStep {
    ...
    pub(super) dependencies: Vec<CanonicalWorkflowDependency>,
}

#[derive(Clone, Debug, Deserialize, PartialEq, Serialize)]
pub(super) struct CanonicalWorkflowDependency {
    pub(super) prerequisite_step_key: String,
    pub(super) release_mode: String,
}

pub(super) type StoredCanonicalWorkflowStep = CanonicalWorkflowStep;
pub(super) type StoredCanonicalWorkflowDependency = CanonicalWorkflowDependency;
...
impl RecoveryAppendSnapshot {
    pub(super) fn into_steps(self) -> Vec<CanonicalWorkflowStep> {
        self.steps.into_iter().map(Into::into).collect()
    }
}

impl From<RecoveryCanonicalWorkflowStep> for CanonicalWorkflowStep {
...
impl From<RecoveryCanonicalWorkflowDependency> for CanonicalWorkflowDependency {
...
) -> CanonicalAppendRequest {
    CanonicalAppendRequest {
        append_window_step_key: Some(append_window_step_key.as_str().to_owned()),
        steps: canonical_workflow_steps(workflow_organization_id, steps),
    }
}

impl CanonicalAppendRequest {
        ...
        .map(|step| {
            let mut dependencies = step
                ...
                .map(|dependency| CanonicalWorkflowDependency {
                    ...
                    release_mode: dependency.effective_release_mode().as_db_value().to_owned(),
                })
                .collect::<Vec<_>>();
            ...
            CanonicalWorkflowStep {
                ...
                execution_kind: step.execution_kind().as_db_value().to_owned(),
                job_type: step.job_type().map(|job_type| job_type.as_str().to_owned()),
                ...
                stage: workflow_step_effective_stage(step).map(str::to_owned),
                allow_handler_continuation: step.allows_handler_continuation(),
                execution_resource_key: step.execution_resource_key().map(str::to_owned),
                dependencies,
            }
        })
        .collect::<Vec<_>>();
...
fn normalize_stored_append_request(
    mut request: CanonicalAppendRequest,
    workflow_organization_id: Option<Uuid>,
) -> CanonicalAppendRequest {
...
fn sort_stored_dependencies(dependencies: &mut [CanonicalWorkflowDependency]) {
    dependencies.sort_by(|left, right| {
        left.prerequisite_step_key
            .cmp(&right.prerequisite_step_key)
            .then(left.release_mode.cmp(&right.release_mode))
...
mod tests {
    use runledger_core::jobs::{
        ...
        WorkflowStepDependencySpec, WorkflowStepEnqueueBuilder, WorkflowType,
    };
    use serde_json::json;
    use sqlx::types::Uuid;
    ...
    #[test]
    fn canonical_snapshot_persists_effective_dependency_release_modes() {
        let payload = json!({"child": true});
        let child = WorkflowStepEnqueueBuilder::new(
            StepKey::new("child"),
        ...
        .set_dependencies([
            WorkflowStepDependencySpec {
                prerequisite_step_key: StepKey::new("omitted"),
                release_mode: None,
            },
            WorkflowStepDependencySpec {
                prerequisite_step_key: StepKey::new("terminal"),
                release_mode: Some(WorkflowDependencyReleaseMode::OnTerminal),
            },
            WorkflowStepDependencySpec {
                prerequisite_step_key: StepKey::new("success"),
        ...
        let snapshot = canonical_append_request(StepKey::new("window"), None, &[child])
            .to_json_value()
            .expect("encode canonical append snapshot");

        assert_eq!(
            snapshot["steps"][0]["dependencies"],
            json!([
                {
                    "prerequisite_step_key": "omitted",
                    "release_mode": "ON_TERMINAL",
                },
                {
                    "prerequisite_step_key": "success",
                    "release_mode": "ON_SUCCESS",
                },
                {
                    "prerequisite_step_key": "terminal",
                    "release_mode": "ON_TERMINAL",
                },
            ])
        );

...
`redacted_fields`; metadata-only reads also omit those columns from the
PostgreSQL projection, while full visibility is an explicit host grant. Job and
workflow list endpoints always return bounded summary shapes. Sensitive stored
fields are selected only for the corresponding detail or history endpoint when
the host grants full visibility. All responses use `Cache-Control: private,
no-store`.
...
resources granted to the request. Metrics, list, and workflow graph responses
...
refresh both committed artifacts. Lint and npm tests fail when either artifact
is stale.
...
integration example. List, detail, aggregate metrics, and capabilities load
once unless the host configures their independent polling intervals.
...
- `202608210001_admin_job_events_history_index` and
  `202608210002_admin_job_logs_history_index` — add `(job_id, id DESC)` indexes
  for bounded newest-first admin history reads. Each index is built
